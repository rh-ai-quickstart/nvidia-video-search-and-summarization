...
from mdx.analytics.core.utils.schema_util import datetime_to_timestamp, dict_frame_to_protobuf_frame
...
class PlaybackFrames:
    ...
    ::
        ...
        playback_app = PlaybackFrames(config_path, playback_filepath, use_simulated_time)
    """
    ...
    def playback(self):
        ...
        try:
            ...
            for loop in range(playloop):
                count = 0
                previous_time_record = 0
                first_record_time = None
                system_offset_in_ms = None
                simulated_offset_in_ms = None
                ...
                with open(self.playback_filepath) as f:
                    for line in f:
                        ...
                        if not data:
                            continue
                        frame = dict_frame_to_protobuf_frame(json.loads(data))

                        if filter_empty_objects and (not frame.objects):
                            continue

                        if count == 0:
                            first_record_time = frame.timestamp
                        ...
                        if x.timestamp.ToMilliseconds() < previous_time_record:
                            logging.error(f"records not sorted, please use sorted file, record index = {count}")
                        previous_time_record = x.timestamp.ToMilliseconds()
...
if __name__ == "__main__":
    ...
    playback_frames_app = PlaybackFrames(args.config, args.playback_filepath)
