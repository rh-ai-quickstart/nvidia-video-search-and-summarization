...
public:
  ...
  Boolean isFrameIdEnabled() { return fFrameIdSupport; }

  void setRxSocketBufSize (int buf_size) { fRxSocketBufSize = buf_size; }
  unsigned int getRxSocketBufSize() { return fRxSocketBufSize; }

  void setTxSocketBufSize (int buf_size) { fTxSocketBufSize = buf_size; }
  unsigned int getTxSocketBufSize() { return fTxSocketBufSize; }

  void useCameraNtpTime(Boolean use_camera_ntp) { fUseCameraNtp = use_camera_ntp; }
  Boolean isCameraNtpUsed() { return fUseCameraNtp; }
  ...
  // Notifies when describes state success or reset
  virtual void sdpReady() {}
  virtual void sdpReset() {}
...
private:
  ...
  Boolean fFrameIdSupport;
  Boolean fUseCameraNtp;
  unsigned int fRxSocketBufSize = 0;
  unsigned int fTxSocketBufSize = 0;
};
...
public:
  ...
  void useCameraNtpTime(Boolean use_camera_ntp) { fUseCameraNtpTime = use_camera_ntp; }
  Boolean isCameraNtpUsed() { return fUseCameraNtpTime; }
...
private:
  ...
  Boolean fUseCameraNtpTime;
  int64_t previous_frameId = 0;
  Boolean fEnableProxyFrameDebug = False;
};
...
public:
  ...
  void setStreamName(char const* streamName) { fstreamName = streamName; }
  const std::string& getStreamName() const { return fstreamName; }
