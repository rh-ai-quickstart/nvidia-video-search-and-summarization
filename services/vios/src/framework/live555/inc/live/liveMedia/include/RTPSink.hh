...
public:
  ...
  // Use rtpBase time provided externally (vod) instead of getTimeOfDay()
  void setRTPBaseTime(const timeval& rtp_base_ts) {
    fRtpBaseTime = rtp_base_ts;
  }
  void setUrl(const std::string& url) { fRTPInterface.setUrl(url); }
...
private:
  ...
  struct timeval fRtpBaseTime = {0,0};
};
