...
public:
  ...
  void useExternalNtpTime (Boolean use_ext_ntp) { fUseExternalNtptime = use_ext_ntp; }
  Boolean isExternalNtpUsed() { return fUseExternalNtptime; }
  void setexternalNtpTime(struct timeval extNtpTimeval) { fExtNtpTimeval = extNtpTimeval; }

  // Use Sender Report base time provided externally (vod) instead of getTimeOfDay()
  void setRTCPBaseTime(const timeval& rtcp_base_ts) {
    fRtcpBaseTime = rtcp_base_ts;
    fUseRtcpBaseTime = True;
  }
...
private:
  ...
  Boolean fUseRtcpBaseTime = False;
  struct timeval fRtcpBaseTime = {0,0};
  struct timeval fLastSrPublishedTime = {0,0};
  ...
  Boolean fUseExternalNtptime;
  struct timeval fExtNtpTimeval = {0,0};
