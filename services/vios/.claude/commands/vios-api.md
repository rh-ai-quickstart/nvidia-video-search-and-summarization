---
name: "vios-api"
description: "Query VIOS (VST) REST APIs: sensor list, recording timelines, video clip extraction, snapshot capture, add/delete sensors and streams"
metadata:
...
**Endpoint Resolution:**
- Use the VIOS endpoint provided by the existing deployment context. This endpoint represents the VST backend reachable from the agent's runtime environment.
- Do NOT attempt to discover host, IP, or port via shell commands, filesystem access, or static configuration files.
- Assume the existing deployment context already provides the correct network endpoint for VST.

**Availability Check:**
- Before making any API call, verify that the VST backend is reachable via the deployment endpoint:
  ```bash
...
```
Response: array of sensor objects. Key fields: `sensorId`, `name`, `location`, `state` (`online`/`offline`/`removed`), `sensorIp`, `hardwareId`, `tags`, `type`, `isTimelinePresent`, `isRemoteSensor`. `state` and `type` are only populated by `GET /sensor/list` — `GET /sensor/{sensorId}/info` does not return them.

`type` enum values: `sensor_onvif`, `sensor_mms_onvif`, `sensor_rtsp`, `sensor_nvstream`, `sensor_udp`, `sensor_webrtc`, `sensor_generic`, `sensor_edge`, `sensor_csi`, `sensor_file`.

> Sensors with `state == 'removed'` (tracked in DB after adaptor removal) return ONLY `sensorId`, `name`, `state`. All other fields (`type`, `sensorIp`, `isTimelinePresent`, `position`, etc.) are absent. Do not assume their presence.

**Get single sensor info:**
```bash
curl -s "http://<VST_ENDPOINT>/vst/api/v1/sensor/<sensorId>/info" | jq .
```
Response: hardware metadata — `sensorId`, `name`, `sensorIp`, `location`, `manufacturer`, `hardware`, `hardwareId`, `firmwareVersion`, `serialNumber`, `tags`, `isRemoteSensor`, `position`, `remoteDeviceId`, `remoteDeviceName`, `remoteDeviceLocation`. Does **not** include `state` or `type` — use `GET /sensor/status` for state, `GET /sensor/list` for type.

> For an unknown `sensorId`, this endpoint returns HTTP 200 with body **`null`** (the 4-byte JSON literal — not an empty body and not an error object). Detect with `jq 'if . == null then "missing" else "ok" end'`. Do NOT rely on HTTP status; do NOT retry. Always verify the sensor exists via `GET /sensor/list` or `/status` before calling.

**Get sensor status (all):**
```bash
curl -s "http://<VST_ENDPOINT>/vst/api/v1/sensor/status" | jq .
```
Response: object keyed by `sensorId`, each with `{name, state, errorCode, errorMessage}`. `state` is only `online` or `offline` at this endpoint (no `removed`).

**Get status of a single sensor:**
```bash
curl -s "http://<VST_ENDPOINT>/vst/api/v1/sensor/<sensorId>/status" | jq .
```
Response: `{name, state, errorCode, errorMessage}`. For an unknown `sensorId`, returns HTTP 200 with `{state: "offline", errorCode: "CameraNotFoundError", errorMessage: ...}` — `name` is absent.
...
```
Response: **array** of stream objects. Per-stream fields: `streamId`, `isMain`, `url`, `vodUrl`, `name`, `type` (e.g. `Rtsp`, `NvStream`), `storageLocation` (`Local`/`Cloud`/`Unknown`), `metadata` with `bitrate`, `codec`, `framerate`, `govlength`, `resolution`. `vodUrl` is only present on RTSP-proxied streams — do not assume it exists.

**Get all streams across all sensors:**
```bash
curl -s "http://<VST_ENDPOINT>/vst/api/v1/sensor/streams" | jq .
```
Response: an **array of single-key objects**, each `{<sensorId>: [stream, ...]}` — NOT a flat map. To convert to a flat map keyed by sensorId, pipe through `jq 'add'`.
...
**Get timeline for a specific stream** — response is a **bare array** `[{startTime, endTime}, ...]`:
```bash
curl -s "http://<VST_ENDPOINT>/vst/api/v1/storage/<streamId>/timelines" | jq .
```

**Get timelines for all streams** — response is an **object** keyed by streamId:
```bash
curl -s "http://<VST_ENDPOINT>/vst/api/v1/storage/timelines" | jq .
```

**Get timelines filtered to specific streams** (same object shape as the multi-stream form above):
```bash
...
> - **Single-stream** `/storage/<streamId>/timelines` → bare array `[{startTime, endTime}, ...]`.
> - **Multi-stream** `/storage/timelines[?streams=…]` → object `{<streamId>: [{startTime, endTime}, ...], ...}`.
> - All times are ISO 8601 UTC. Streams with no recordings are omitted from the multi-stream object form.

**Get storage usage (per-stream and totals):**
```bash
curl -s "http://<VST_ENDPOINT>/vst/api/v1/storage/size" | jq .
```
Response: object keyed by `streamId`, each with `{sizeInMegabytes, state}` (state ∈ `connected | disconnected | recording`), plus a `total` key with `{sizeInMegabytes, totalDiskCapacity, totalAvailableStorageSize, remainingStorageDays}`. Sizes are in megabytes; capacity values are in bytes — do NOT compare directly without unit conversion.
...
Optional fields for both: `hardware`, `manufacturer`, `serialNumber`, `firmwareVersion`, `hardwareId`, `tags`, `verifyRtsp` (boolean, default `false`).

**Server-applied defaults** when fields are missing:
- `username`/`password`: optional — the server accepts the request without them, but the sensor will be added unauthorized (call `POST /sensor/<sensorId>/credentials` later to authorize).
- `location`, `hardware`, `manufacturer`, `serialNumber`, `firmwareVersion`, `hardwareId`: default to the literal string `"Unknown"` on RTSP add.
- `name`: an auto-generated unique name (e.g. `Camera-<N>`) is assigned if you don't provide one.
- `sensorId`: always a freshly generated UUID — any client-supplied `sensorId` in the body is ignored for non-remote sensors.
- `verifyRtsp: true` runs an RTSP DESCRIBE probe before persisting (fails fast with `InvalidParameterError`). **Ignored on NVStreamer deployments.**

**Common errors (`error_code`):**
- `InvalidParameterError`, `Sensor exists already, sensorId: <id>, sensorName: <name>` — duplicate detected.
- `InvalidParameterError`, `User given name is invalid or already exists` — name collision (case-sensitive). On NVStreamer, names containing whitespace are also rejected.
- `VMSNotSupportedError`, `Sensors count limit reached` — deployment at `maxSensorsSupported`.

**Trigger network scan for sensors:**
```bash
curl -s -X POST "http://<VST_ENDPOINT>/vst/api/v1/sensor/scan" | jq .
```
Returns no body on success. Async — discovered sensors appear in `/sensor/list` shortly after.
...
```bash
# Returns the JSON literal `true` on success
...
- **Unknown sensorId**: returns HTTP 200 with `{error_code: "CameraNotFoundError", ...}` — check the response body, not the status code.
- **Response body** is the JSON literal `true` on success — not an object.
...
**Upload error status codes:**
- `409` — filename already exists (PUT v2 only; v1 auto-renames)
- `415` — container format is unsupported
- `422` — codec is unsupported
- `507` — disk full
...
# Use startTime=*&endTime=* to mean unbounded (delete all segments)
...
> For NvStreamer file-sensor deployments, calling `DELETE /storage/file/<streamId>` with no time range deletes the uploaded file outright (sensor + on-disk file). For non-NvStreamer deployments, `startTime` and `endTime` are required and define the recording range to delete.
...
- **Endpoint resolution:** The VST endpoint is provided by the existing deployment context. Do not attempt manual IP/port discovery. If unavailable, ask the user. All curl examples use `<VST_ENDPOINT>` as a placeholder — substitute the resolved endpoint before executing.
...
**Start a WebRTC live stream** (client posts SDP offer, gets answer + `mediaSessionId`):
```bash
...
**Client polls for server-side ICE candidates** (call repeatedly during connection setup):
```bash
...
**setAnswer** (only used when the server initiated the offer over a websocket — most clients skip this). `sessionDescription` is an **object**, not a string:
```bash
...
**Start a VOD WebRTC stream** (window defined by `startTime`/`endTime`):
```bash
...
**Post / poll ICE candidates** (same shape as live — see Section 12). Replace `/live/` with `/replay/` in the URL.
...
> - `GET /sensor/version`, `/live/version`, `/replay/version` → `{type, version}` (e.g. `{"type": "vst", "version": "2.1.0-26.05.2"}`)
...
- NOTE on response `Content-Type`: the server emits `Content-Type: text/plain` on every JSON response (the body is JSON; only the header is mis-set). Always parse the body with `jq` or a JSON parser regardless of the wire `Content-Type` header. Do NOT filter responses on `Content-Type: application/json` because they will never match. The only exceptions are snapshot endpoints (`image/jpeg`) and clip download endpoints (`application/octet-stream` or `video/mp4`), which set their own correct Content-Type.
