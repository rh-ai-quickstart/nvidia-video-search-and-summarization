...
typedef struct jpeg_mgr j_mgr;
typedef struct jpeg_mgr * j_mgr_ptr;
#endif
...
#define jpeg_common_fields \
  ...
  void * jpegMgr; /* Tegra related manager */\
  ...
  int gpu_id; /* Used in transform */ \
  unsigned int quality; \
  int dec_out_mem_type; /* NvBufSurfaceMemType for decoder output. Only applicable for jcuvid */ \
  int enc_in_mem_type; /* NvBufSurfaceMemType for encoder input. Only applicable for jcuvid */ \
  boolean dec_use_external_buffer; /* Use external buffer for decoder output */ \
  unsigned char reserved[256] /* Reserved field for future use */
#elif GPU_ACCELERATE
#define jpeg_common_fields \
  struct jpeg_error_mgr * err;	/* Error handler module */\
  struct jpeg_memory_mgr * mem;	/* Memory manager module */\
